...
- (void)initializePageControl {
    ...
    _pageControl.center = CGPointMake(CGRectGetWidth(_scrollView.frame)*0.5, CGRectGetHeight(_scrollView.frame) - 15);
...
- (void)loadData {
    ...
    if (_datasourceImages.count == 0) {
        ...
        if ([self.datasource respondsToSelector:@selector(placeHolderImageOfZeroBannerView)]) {
            ...
            //修改
            if ([self.datasource respondsToSelector:@selector(contentModeForImageIndex:)]) {
                imageView.contentMode = [_datasource contentModeForImageIndex:0];
            }
        }
        return;
    }
...
//                [imageView sd_setImageWithURL:[imageSource isKindOfClass:[NSString class]] ? [NSURL URLWithString:imageSource] : imageSource placeholderImage:placeHolderImage];
                //修改
                [[SDWebImageManager sharedManager] downloadImageWithURL:[imageSource isKindOfClass:[NSString class]] ? [NSURL URLWithString:imageSource] : imageSource options:SDWebImageRetryFailed progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                    //
                    if (image) {
                        imageView.image = image;
                        imageView.contentMode = [self.datasource contentModeForImageIndex:i];;
                    } else {
                        if ([self.datasource respondsToSelector:@selector(contentModeForImageIndex:)]) {
    ...
    [path moveToPoint:CGPointMake(-5, _scrollView.height + 1)];
    [path addQuadCurveToPoint:CGPointMake(kDeviceWidth + 5, _scrollView.height + 1) controlPoint:CGPointMake((kDeviceWidth + 10)/2.0, _scrollView.height - 30)];
    CAShapeLayer *shapeLayer = [CAShapeLayer layer];
    shapeLayer.path = path.CGPath;
    shapeLayer.fillColor = [UIColor whiteColor].CGColor;
    [self.layer addSublayer:shapeLayer];
    [self bringSubviewToFront:self.pageControl];*/
    self.pageControl.currentPageIndicatorTintColor = ColorWithHex(0x52ADFF, 1);
    self.pageControl.pageIndicatorTintColor = ColorWithHex(0xffffff, 1);
